...
def testMemoryMlp():
    # masker = None
    masker = Masker(prune_ratio=0.85)

    model = nn.Sequential(*[LinearSparse(384, 384, quantize=False, masker=masker) for _ in range(10)])
...
def testMesaMlp():
    ...
    model = nn.Sequential(*[mesa.Linear(384, 384) for _ in range(10)])
...
def testMlpStoreActivationPrune():
    ...
    fc2_grad_origin = mlp_origin[1].weight.grad
    ...
    fc2_grad_our = mlp_our[1].weight.grad
