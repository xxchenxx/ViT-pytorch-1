...
def testMemoryGELU():
    ...
    # model = nn.Sequential(*[nn.Linear(384, 384)]+[GELUSparse(quantize=False, masker=masker) for _ in range(10)])
    model = nn.Sequential(*[nn.Linear(384, 384)] + [nn.GELU() for _ in range(10)])
...
def testMlpStoreActivationPrune():
    mlp_origin = nn.Sequential(*[nn.Linear(384, 384)] + [nn.GELU() for _ in range(1)]).cuda()

    masker = Masker(prune_ratio=0.0)
    mlp_our = nn.Sequential(*[nn.Linear(384, 384)]+[GELUSparse(quantize=False, masker=masker) for _ in range(1)]).cuda()
