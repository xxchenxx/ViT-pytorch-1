...
from models.modeling_attn_store_prune import AttentionStoreActivationPrune, MlpActivationPrune
...
class Attention(nn.Module):
    def __init__(self, config, vis, prune_mode=False, prune_after_softmax=False, n_tokens=1):
        ...
        self.query = ms.Linear(config.hidden_size, self.all_head_size, quant_groups=self.num_attention_heads)
        self.key = ms.Linear(config.hidden_size, self.all_head_size, quant_groups=self.num_attention_heads)
        self.value = ms.Linear(config.hidden_size, self.all_head_size, quant_groups=self.num_attention_heads)

        self.out = ms.Linear(config.hidden_size, config.hidden_size, quant_groups=self.num_attention_heads)
        self.attn_dropout = Dropout(config.transformer["attention_dropout_rate"])
        self.proj_dropout = Dropout(config.transformer["attention_dropout_rate"])

        self.mm1 = ms.MatMul(quant_groups=self.num_attention_heads)
        self.mm2 = ms.MatMul(quant_groups=self.num_attention_heads)
        ...
        self.softmax = ms.Softmax(dim=-1, quant_groups=self.num_attention_heads)
...
class Mlp(nn.Module):
    def __init__(self, config):
        ...
        self.num_attention_heads = config.transformer["num_heads"]
        self.fc1 = ms.Linear(config.hidden_size, config.transformer["mlp_dim"], quant_groups=self.num_attention_heads)
        self.fc2 = ms.Linear(config.transformer["mlp_dim"], config.hidden_size, quant_groups=self.num_attention_heads)
...
class Block(nn.Module):
    def __init__(self, config, vis, prune_mode=False, prune_after_softmax=False, n_tokens=1,
        ...
        self.attention_norm = ms.LayerNorm(config.hidden_size, eps=1e-6)
        self.ffn_norm = ms.LayerNorm(config.hidden_size, eps=1e-6)
...
class Encoder(nn.Module):
    def __init__(self, config, vis, prune_mode=False, prune_after_softmax=False, n_tokens=1, **block_kwargs):
        ...
        self.encoder_norm = ms.LayerNorm(config.hidden_size, eps=1e-6)
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False,
        ...
        self.head = ms.Linear(config.hidden_size, num_classes)
